...
FITNESS FOR A PARTICULAR PURPOSE, FILE AND NON-INFRINGEMENT. IN NO EVENT
...
static INLINE unsigned nedtriebitscanr(size_t value)
{
  ...
  _BitScanReverse64(&bitpos, value | 1);
#else
  assert(4==sizeof(size_t));
  _BitScanReverse(&bitpos, value | 1);
#endif
  return (unsigned) bitpos;
#elif defined(__GNUC__)
  return sizeof(value)*__CHAR_BIT__ - 1 - (unsigned) __builtin_clz(value | 1);
#else
...
namespace nedtries {
  ...
  */
#if __NO__cplusplus > 199711L || defined(HAVE_CPP0X) /* Do we have C++0x? */
